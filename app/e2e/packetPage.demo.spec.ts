...
test.describe("Demo packet page", { tag: TAG_DEMO_PACKETS }, () => {
  ...
  test("can see packet group name and packet id", async ({ page }) => {
    ...
    await expect(content.getByRole("heading", { name: "parameters", level: 2 })).toBeVisible();
    await expect(content.getByText(parametersPacketId)).toBeVisible();
  });

  test("can see packet group display name and description", async ({ page }) => {
    await page.goto(`./custom_metadata/${customMetadataPacketId}`);
    const content = await getContentLocator(page);
    await expect(content.getByRole("heading", { name: "Packet with description", level: 2 })).toBeVisible();
    await expect(content.getByText(customMetadataPacketId)).toBeVisible();
    await expect(content.locator("span").filter({ hasText: "custom_metadata" })).toBeVisible();
    await expect(content.getByText("A longer description")).toBeVisible();
  });
  ...
  test.describe("Summary", () => {
    ...
    test("can see dependencies", async ({ page, baseURL }) => {
      ...
      const dep2ListItem = depItems.last();
      ...
      const noneDiv = dep1DependsDiv.locator(".italic");
    ...
    test("can see reports", async ({ page }) => {
      ...
      const reportFrame = reportsDiv.locator("iframe");
      await expect(reportFrame).toBeVisible();
      await expect(reportFrame.contentFrame().getByRole("heading")).toHaveText("TEST");
    });
  });
  ...
  test.describe("Metadata", () => {
    ...
    test("can see timings", async ({ page }) => {
      ...
      await expect(timingItems.first()).toHaveText("StartedThu, 29 Aug 2024 18:54:40 GMT");
      await expect(timingItems.last()).toHaveText("Elapsed192 milliseconds");
    });
    ...
    test("can see packages", async ({ page }) => {
      ...
      expect(listItems.length).toBe(21);
