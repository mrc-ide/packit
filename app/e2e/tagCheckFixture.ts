...
const checkDemoPacketsTag = (tags: string[], baseURL: string) => {
    if (tags.includes(TAG_DEMO_PACKETS)) {
        ...
        console.warn(`Running test expecting demo packets against non-local server ${baseURL}. Consider running playwright with ${safeGrep}`);
    }
}
...
export const test = base.extend<{ tagCheck: void }>({
    tagCheck: [async ({ baseURL }, use, testInfo) => {
