...
import { getCurrentUser, removeCurrentUser } from "./localStorageManager";
...
export class ApiService implements API {
    ...
    axiosInstance.interceptors.request.use((config) => {
      if (validateToken(getCurrentUser() || this.injectedStore.getState().login.user)) {
        config.headers.authorization = `Bearer ${
          getCurrentUser().token || this.injectedStore.getState().login.user.token
        }`;
      }
      return config;
    });
  ...
  private handle401Error = (error: AxiosError) => {
    if (error.response && error.response.status === 401 && getCurrentUser()) {
      removeCurrentUser();
